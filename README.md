...
### 6. `POST /api/youtube/upload`
AI 직캠 영상을 유튜브 쇼츠로 즉시 업로드합니다.
- **Request (JSON):**
  - `video_path`: `/outputs/...mp4` 혹은 5번에서 받은 경로 파일명
  - `title`: 유튜브 영상 제목 (기본값: Gemini Fancam Shorts)
  - `description`: 영상 설명글 (기본값: Generated by Gemini AI #Shorts)
  - `privacy`: "public" (기본값), "unlisted", "private"
- **Response (JSON):**
  - `video_id`: 유튜브 고유 비디오 ID 반환
  - `url`: 시청 가능한 유튜브 URL 주소 반환

### 7. `POST /api/youtube/recommend`
영상 정보(타겟 설명)를 바탕으로 Gemini AI가 유튜브 쇼츠 제목과 해시태그 설명을 추천해줍니다.
- **Request (JSON):**
  - `target_description`: 영상의 타겟 정보 (예: "푸른 모자를 쓴 춤추는 사람")
- **Response (JSON):**
  - `title`: AI 추천 유튜브 쇼츠 제목
  - `description`: 추천된 유튜브 설명 (해시태그 포함)
